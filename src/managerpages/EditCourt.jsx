...
import { getCourts, updateCourt } from '../api/courtAPI';
...
const EditCourt = () => {
  ...
  useEffect(() => {
    const fetchCourt = async () => {
      try {
        ...
        const court = response.find((court) => court.id == id);
        console.log(court);
        setCourt(court);
      } catch (error) {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-gray-100 rounded-lg shadow-md">
      <h2 className="text-3xl font-bold mb-6 text-center text-blue-700">Chỉnh sửa sân cầu lông</h2>
